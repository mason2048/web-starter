...
6. 下载并校验固定的 Docker Compose 5.3.1 独立二进制后，将同一字节副本以 fail-closed 方式安装为当前发布任务的用户级 Docker CLI 插件；外层生产模型展开、生成项目、恢复演练和全部嵌套运行验收因此使用同一版本。这样显式的 `bind.create_host_path: false` 不会被旧版 Compose JSON 序列化器静默省略，TLS 挂载策略仍严格拒绝任何缺失该字段的模型。随后使用本次实际 App/Nginx digest 和显式配置的 MySQL/Redis digest 展开 `compose.production.yaml` 并执行生产 Compose 策略；缺少 `WEB_STARTER_MYSQL_IMAGE` 或 `WEB_STARTER_REDIS_IMAGE` 仓库变量即失败。
