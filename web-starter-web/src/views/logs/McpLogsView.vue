  ...
  <div class="page-content">
    ...
    <div class="table-panel">
      <el-table v-loading="loading" :data="records" row-key="id" empty-text="暂无 MCP 调用日志">
        ...
        <el-table-column prop="traceId" label="Trace ID" min-width="170" show-overflow-tooltip><template #default="{ row }"><span class="mono">{{ row.traceId || '—' }}</span></template></el-table-column>
...
const query = reactive<{ page: number; size: number; keyword: string; toolName: string; success: boolean | '' }>({ page: 1, size: 10, keyword: '', toolName: '', success: '' })
